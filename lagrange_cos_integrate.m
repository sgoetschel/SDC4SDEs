%%Computes the integration matrix including the cosine function
%by Lisa Fischer
function [entry] = lagrange_cos_integrate( t, upper_bound, deltaT, c, k)
...
for l=n+1:-1:1
    ...
    for r=0:l-1
        ...
        int_upper = t(upper_bound)^(l-1-r) *sin(k*pi/deltaT *t(upper_bound)+r*pi/2);
        %and t_{i-1}
        int_lower = t(upper_bound-1)^(l-1-r) *sin(k*pi/deltaT *t(upper_bound-1)+r*pi/2);
